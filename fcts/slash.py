...
class Slash(commands.Cog):
    ...
    async def add_tag(self, ctx: SlashContext, name: str, answer: str):
        if len(name.split()) > 1:
            ...
            return
        check_exists = await self.db_get_command(ctx.guild.id, name)
        if check_exists is not None:
            await ctx.send("Une commande existe déjà avec ce nom !")
            return
        desc = "something"
        ID = await self.disc_add_command(ctx.guild.id, name, desc)
        await self.db_add_command(ID, ctx.guild.id, name, desc, answer)
    ...
    async def remove_tag(self, ctx: SlashContext, name: str):
        ...
        if check_exists is None:
            ...
            return
        if await self.disc_delete_command(ctx.guild.id, check_exists['ID']):
            await self.db_delete_command(ctx.guild.id, check_exists['ID'])
            await ctx.send(f"La commande `/{check_exists['name']}` a bien été supprimée !")
        else:
    ...
    async def disc_delete_command(self, guildid: int, ID: int):
        ...
        if not 200 <= r <= 204:
            ...
            return False
        self.bot.slashClient.commands = {k: v for k, v in self.bot.slashClient.commands if v['id'] != ID}
        return 200 <= r <= 204
